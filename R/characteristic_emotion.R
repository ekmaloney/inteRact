...
characteristic_emotion <- function(id_info = NULL,
  ...
  if(is.null(id_info)){
    ...
    R <- eq %>%
        dplyr::filter(.data$M == "000" & .data$I != "000") %>%
        dplyr::select(.data$postME:.data$postMA) %>% as.matrix() %>% t()
    ...
    #NOW TERM 1
        E <- eq %>%
            ...
            mutate(term = dplyr::case_when(MA == 1 ~ "MA",
                                    ME == 1 ~ "ME",
                                    MP == 1 ~ "MP"),
                   term = factor(.data$term, levels = c("ME", "MP", "MA"))) %>%
                arrange(.data$term) %>%
            dplyr::select(.data$postME:.data$postMA) %>% as.matrix() %>% t()

        QE <- eq %>%
            dplyr::filter(.data$interaction_term == 1 & .data$IE == 1) %>%
            mutate(term = dplyr::case_when(MA == 1 ~ "MA",
                                           ME == 1 ~ "ME",
                                           MP == 1 ~ "MP"),
                   term = factor(.data$term, levels = c("ME", "MP", "MA"))) %>%
        ...
        QP <- eq %>%
            dplyr::filter(.data$interaction_term == 1 & .data$IP == 1) %>%
            mutate(term = dplyr::case_when(MA == 1 ~ "MA",
                                           ME == 1 ~ "ME",
                                           MP == 1 ~ "MP"),
                   term = factor(.data$term, levels = c("ME", "MP", "MA"))) %>%
        ...
        QA <- eq %>%
            dplyr::filter(.data$interaction_term == 1 & .data$IA == 1) %>%
            mutate(term = dplyr::case_when(MA == 1 ~ "MA",
                                           ME == 1 ~ "ME",
                                           MP == 1 ~ "MP"),
                   term = factor(.data$term, levels = c("ME", "MP", "MA"))) %>%
